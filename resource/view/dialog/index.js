...
var confirm_vue = {
  template:'<div class="mask" v-if="visible">'
            ...
            +'<div class="confirm" ref="confirm">'
              +'<div class="confirm-head" v-if="baseOpt.showHead">'
                +'<span class="confirm-title">{{baseOpt.title}}</span>'
                +'<span class="confirm-tool" v-if="baseOpt.slot.tool.length>0"></span>'
                +'<span class="icon icon-delete confirm-close" v-if="baseOpt.showClose" @click="visible=false"></span>'
              +'</div>'
              +'<div class="confirm-content"></div>'
              +'<div class="confirm-foot">'
                +'<span class="btn" @click="baseOpt.failFn" v-if="!baseOpt.onlySure">{{baseOpt.btns[0]}}</span>'
                +'<span class="btn btn-blue" @click="baseOpt.okFn">{{baseOpt.btns[1]}}</span>'
              +'</div>'
            +'</div></div>',
  data:function(){
    var self = this;
  ...
  watch:{
    'visible':function(nv,ov){
      if(nv && this.baseOpt.lock){//设置锁定并且弹框已经出现
  ...
  methods:{
   ...
   renderConfirm:function(){
    //渲染尺寸
    this.$refs.confirm.style.width = this.baseOpt.width;
    //渲染内容主体
    this.$refs.confirm.querySelector('.confirm-content')[this.baseOpt.dangerouslyUseHTMLString?'innerHTML':'textContent'] = this.baseOpt.content
    //底部按钮布局
    this.$refs.confirm.querySelector('.confirm-foot').classList.add("align-"+this.baseOpt.align)
    //渲染工具栏
    if(this.baseOpt.slot.tool.length>0){
      this.$refs.confirm.querySelector('.confirm-tool').innerHTML = this.baseOpt.slot.tool
    }
   }
  }
}
